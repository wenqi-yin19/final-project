<?php 
	...
	if($_REQUEST["type"]=="logout"){
		session_destroy();
		session_start();
		urlMsg("Exit Successfully", __BASE__."/leftlogin.php");
		die;
	}
	...
	if ($_POST) {
			...
			if ($rsRow['password'] == $_POST["password"]){
				...
				urlMsg("Login Successfully", __BASE__."/leftusercenter.php");
				die;
			} else {
                ...
                <form class="form-horizontal" method="post" role="form" name="form1"  action="?" onSubmit="return check();">
                    ...
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-10">
                            <button type="submit" class="btn btn-default">Submit</button> <a href="leftforgetpwd.php"><button type="button" class="btn btn-default">Forget password</button></a>
                        </div>
                    </div>
                </form>
				</div>
